...
public class IdeController {
    ...
    abstract static class IdeDriveImage implements IdeDrive {
        ...
        protected void init() {
            ...
            setupGeometry();
            logger.debug("Geometry: cylinders: {}, heads: {}, sectors: {}. Total LBA sectors: {}",
        ...
        private boolean setupAltProGeometry() {
            ...
            try {
                ...
                logger.debug("Can't set up AltPro geometry: {}", e.getMessage());
            }
            return false;
        }
        ...
        public void detach() {
            try {
                ...
                logger.warn("Error while detaching image " + image.getLocation(), e);
            }
        }
        ...
        public int read(byte[] buffer, long sectorIndex, int numSectors) {
            ...
            try {
                ...
                logger.error("Can't read image at position " + position, e);
                return -1;
            }
            return numSectors;
        }
        ...
        public int write(byte[] buffer, long sectorIndex, int numSectors) {
            ...
            try {
                ...
                logger.error("Can't write image at position " + position, e);
                return -1;
            }
            return numSectors;
        }
        ...
        void handleCommand(int command) {
            switch (command) {
                ...
                case IdeInterface.CMD_EXEC_DIAGNOSTIC:
                    ...
                    break;
                default:
                    logger.warn(String.format("Unsupported command: 0x%02X", command & 0xFF));
                    abortCommand();
            }
        }
