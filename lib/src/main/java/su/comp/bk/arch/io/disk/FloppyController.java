...
import su.comp.bk.util.SparseBooleanArray;
...
public class FloppyController implements Device {
    ...
    class FloppyDrive {
        ...
        private void saveCurrentTrackData() throws IOException {
            ...
            while (true) {
                ...
                if (trackNumber != currentTrackNumber) {
                    logger.warn("Unexpected track number: expected: {}, found: {}",
                            currentTrackNumber, trackNumber);
                    continue;
                }
                int trackSide = data & 0377;
                if (trackSide != currentTrackSide.ordinal()) {
                    logger.warn("Unexpected track side: expected: {}, found: {}",
                ...
                if (sectorSize != 2) {
                    logger.warn("Unexpected sector size: {}", sectorSize);
                    continue;
                }
                ...
                if (crcValue != (Crc16Utils.calculate(trackData, headerPosition, length) & 0177777)) {
                    logger.warn("Invalid sector header CRC, sector: {}", sectorNumber);
                    continue;
                }
                ...
                if (crcValue != (Crc16Utils.calculate(trackData, dataPosition, length) & 0177777)) {
                    logger.warn("Invalid sector data CRC, sector: {}", sectorNumber);
                    continue;
                }

                // Save sector data to image
                logger.debug("Saving sector data, sector number: {}", sectorNumber);
                for (int wordIndex = 0; wordIndex < WORDS_PER_SECTOR; wordIndex++) {
         ...
         */
        void mountDiskImage(DiskImage diskImage, boolean isWriteProtectMode) throws Exception {
            // Check disk image
